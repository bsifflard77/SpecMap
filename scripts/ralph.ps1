...
6. If ALL verifications PASS:
   ...
   - Update PRD.md: If all story criteria [x], mark story checkmark
...
7. If ANY verification FAILS:
   ...
   - Do NOT commit
   - Update progress.md:
     - Add iteration log entry (status: X)
...
- If ALL stories show checkmark: Output <promise>COMPLETE</promise>
...
function Register-ExitPush {
    if (-not $script:exitPushRegistered) {
        # Note: Register-EngineEvent may not work in all contexts
        # Using try/finally in main loop as backup
...
function Is-StuckOnSameError {
    ...
    # Simple heuristic: check if we've seen similar failure patterns
    # This could be enhanced with more sophisticated error tracking
    return $false
}
...
function Invoke-ClaudeIteration {
    ...
    # Invoke Claude Code with the iteration prompt
...
function Test-PRDComplete {
    ...
    # Check if any unchecked criteria remain
    if ($content -match '\[ \]') {
        return $false
    }
...
try {
    ...
    if (-not (Test-Path $ProgressPath)) {
        ...
        # In a full implementation, we'd generate progress.md from PRD.md here
    ...
    # Track previous story for push triggers
    $previousStory = $null
    ...
    for ($i = 1; $i -le $MaxTotalIterations; $i++) {
        ...
        if ($Skip -and $currentStory -eq $Skip) {
            ...
            # Would need to advance to next story in progress.md
            continue
        }

        # Check per-story attempts
        ...
        # Check if PRD shows all complete
        if (Test-PRDComplete) {
            Push-ToRemote -Reason "all stories complete"
            Show-CompletionSummary
            exit 0
