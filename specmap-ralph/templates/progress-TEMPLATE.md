# Progress

**Feature**: [Feature Name]
**PRD**: PRD.md
**Started**: [YYYY-MM-DD HH:MM:SS]
**Last Updated**: [YYYY-MM-DD HH:MM:SS]
**Status**: Ready | In Progress | Complete | Paused

---

## Cold Start Briefing

> Read this first. It tells you exactly where to pick up.

**Story**: US-[XXX] - [Story Title]
**Criterion**: [The specific criterion being worked on]
**State**: [What's done, what's not]
**Next Action**: [Literal next step to take]
**Files**: `path/to/file.ts`
**Rollback**: [commit hash] - [commit message snippet] | None yet
**Last Push**: [YYYY-MM-DD HH:MM:SS] (after [story/event]) | None yet

---

## Learnings

> Patterns and gotchas discovered. Read BEFORE starting work.

- [Learning 1] (discovered [YYYY-MM-DD HH:MM])

<!--
LEARNING RULES:
- Be SPECIFIC: Not "be careful" but "DatePicker expects ISO format"
- Be ACTIONABLE: How to apply the learning
- Include TIMESTAMP: When it was discovered
-->

---

## Iteration Log

### #1 - [YYYY-MM-DD HH:MM:SS]
**Story**: US-[XXX] | **Criterion**: [Which one]
**Status**: Complete | Failed | Paused
**Duration**: [X] min
**Commit**: `[commit message]` | None (failed)
**Notes**: [Brief notes, what was learned]

<!--
LOG RULES:
- Log EVERY iteration, success or failure
- Keep last 5-10 iterations here
- Archive older ones to progress.archive.md if needed
-->

---

## Story Summary

| Story | Title | Status | Iterations |
|-------|-------|--------|------------|
| US-001 | [Title] | Pending | - |
| US-002 | [Title] | Pending | - |

<!--
STATUS KEY:
- Pending
- In Progress
- Complete
- Blocked
- Paused
-->
